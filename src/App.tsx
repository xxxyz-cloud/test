...
const AGENTS = [
  {
    ...
    desc: "Reads your spec/PDF and extracts tech stack, hardest parts, what's already done, and your demo escape strategy. All agents build on this.",
    accent: "text-red-400" as const, icon: ShieldAlert,
    tags: ["Deep Analysis", "PDF-Aware", "Intel Backbone"],
  ...
  },
  {
    ...
    desc: "Triages your features with specific reasons — referencing your actual tech stack. Tells you exactly how to shortcut each kept feature.",
    accent: "text-orange-400" as const, icon: Sparkles,
    tags: ["Scope Triage", "Shortcuts", "+70% Odds"],
  ...
  },
  {
    ...
    desc: "Generates specific, actionable hour-by-hour tasks — not 'Build core feature' but 'Scaffold auth route with hardcoded JWT, skip validation'.",
    accent: "text-blue-400" as const, icon: Compass,
    tags: ["Concrete Tasks", "Execution", "Tech-Specific"],
  ...
  },
  {
    ...
    desc: "Simulates two parallel futures — original plan vs Phoenix plan — with project-specific events, not generic warnings.",
...
function AnimatedChance({ before, after }: { before: number; after: number }) {
  ...
  React.useEffect(() => {
    ...
    const timer = setInterval(() => {
      count++;
      if (count >= steps) { setCurrentBefore(safeBefore); setCurrentAfter(safeAfter); clearInterval(timer); }
      else { setCurrentBefore(Math.round((safeBefore / steps) * count)); setCurrentAfter(Math.round((safeAfter / steps) * count)); }
    }, 1200 / steps);
    ...
    <div className="bg-zinc-950/60 p-5 rounded-xl border border-zinc-800/80 mt-6 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
        ...
        <div className="text-center bg-emerald-950/15 p-4 rounded-lg border border-emerald-500/20 relative">
          ...
          <span className="block text-[10px] font-mono text-emerald-400 uppercase tracking-wider mb-1">Phoenix Plan</span>
          <div className="text-4xl font-extrabold text-emerald-400 leading-none">{currentAfter}%</div>
          <span className="text-[10px] text-emerald-500/80 block mt-1.5 font-mono font-semibold">Highly survivable</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <section className="w-full py-16 px-8 relative">
      <div className="max-w-4xl mx-auto">
        ...
        <h2 className="text-4xl md:text-5xl font-bold tracking-tight leading-[1.1] mb-14">
        ...
        <div className="flex flex-col">
          {AGENTS.map((agent) => {
            ...
            return (
              <div key={agent.num} className="group relative grid grid-cols-[52px_1fr_44px] gap-x-5 py-7 border-b border-white/[0.06] first:border-t first:border-white/[0.06] overflow-hidden cursor-default">
                <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none bg-gradient-to-r ${agent.glow} to-transparent`} />
                <span className="relative font-mono text-[11px] tracking-[0.12em] text-white/20 group-hover:text-white/45 transition-colors duration-300 pt-1.5 select-none">{agent.num}</span>
                <div className="relative flex flex-col">
                  <span className={`text-[clamp(20px,3.2vw,34px)] font-semibold tracking-[-0.03em] leading-none text-white/45 transition-colors duration-300 group-hover:text-zinc-100 ${agent.hoverName}`}>{agent.name}</span>
                  <p className="text-[13px] leading-relaxed text-white/38 max-w-[500px] overflow-hidden transition-all duration-[380ms] ease-[cubic-bezier(0.4,0,0.2,1)]"
                    style={{ maxHeight: 0, opacity: 0, marginTop: 0 }}
                    ref={(el) => {
                      if (!el) return;
                      const parent = el.closest(".group");
                      if (!parent) return;
                      const show = () => { el.style.maxHeight = "80px"; el.style.opacity = "1"; el.style.marginTop = "8px"; };
                      const hide = () => { el.style.maxHeight = "0"; el.style.opacity = "0"; el.style.marginTop = "0"; };
                      parent.addEventListener("mouseenter", show);
                      parent.addEventListener("mouseleave", hide);
                    }}>{agent.desc}</p>
                  <div className="flex flex-wrap gap-1.5 overflow-hidden transition-all duration-[380ms] ease-[cubic-bezier(0.4,0,0.2,1)] delay-[50ms]"
                    style={{ maxHeight: 0, opacity: 0, marginTop: 0 }}
                    ref={(el) => {
                      if (!el) return;
                      const parent = el.closest(".group");
                      if (!parent) return;
                      const show = () => { el.style.maxHeight = "40px"; el.style.opacity = "1"; el.style.marginTop = "10px"; };
                      const hide = () => { el.style.maxHeight = "0"; el.style.opacity = "0"; el.style.marginTop = "0"; };
                      parent.addEventListener("mouseenter", show);
                      parent.addEventListener("mouseleave", hide);
                    }}>
                ...
                <div className="relative flex items-center justify-center w-10 h-10 rounded-[10px] opacity-[0.14] group-hover:opacity-95 group-hover:scale-110 transition-all duration-300 mt-0.5 flex-shrink-0" style={{ background: agent.iconBg }}>
        ...
        <div className="mt-14 grid grid-cols-3 border border-white/[0.07] rounded-2xl overflow-hidden">
          {[
            { value: "15→85%", label: "Success rate jump", dot: "bg-red-500/50" },
            { value: "< 2s",   label: "Per agent response", dot: "bg-orange-500/50" },
            { value: "Cached", label: "Demo-day fast path", dot: "bg-blue-500/50" },
          ].map((s, i) => (
            <div key={i} className={`relative p-6 ${i < 2 ? "border-r border-white/[0.07]" : ""}`}>
              <div className={`absolute top-5 right-5 w-1.5 h-1.5 rounded-full ${s.dot}`} />
              <div className="text-[26px] font-bold tracking-[-0.04em] text-zinc-100 leading-none mb-1.5">{s.value}</div>
              <div className="text-[11px] font-mono tracking-[0.1em] uppercase text-white/30">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <ErrorBoundary>
      <AnimatePresence mode="wait">
        {showLanding ? (
          <motion.div key="landing" initial={{ opacity: 1 }} exit={{ opacity: 0, y: -24 }} transition={{ duration: 0.45, ease: "easeInOut" }}>
            <LandingPage onEnter={handleEnter} />
          </motion.div>
        ) : (
          <motion.div key="app" initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: "easeInOut" }} onAnimationComplete={() => ScrollTrigger.refresh()}>
        ...
        <div className="max-w-5xl mx-auto px-4 md:px-8 pb-10">
          ...
          <section className="mb-8">
            <div className="flex items-center gap-1.5 text-zinc-500 text-[10px] font-mono uppercase tracking-[0.2em] mb-3">
              <Sparkles className="w-3.5 h-3.5 text-orange-400" /><span>Select a crisis scenario preset</span>
            </div>
                ...
                <AnimatePresence>
                  {intelligence && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                      ...
                      <p className="text-[10px] text-zinc-600 mt-1.5">Agents 2 + 3 will triage this list and build your hour-by-hour plan in one shot.</p>
                    </motion.div>
                  )}
                </AnimatePresence>
            ...
            <aside className="lg:col-span-5 space-y-4">
              ...
              <div className="bg-zinc-900/10 border border-zinc-900 p-4 rounded-2xl flex items-start gap-2.5 text-left">
                ...
                <div className="text-[11px] text-zinc-600 leading-normal">
                  <span className="font-semibold text-zinc-500">How it works:</span> Agent 1 reads your goal + PDF and builds a project intelligence report. Agents 2+3 fire together to triage features and build your hour-by-hour plan. Agent 4 simulates both futures.
                </div>
              </div>
            </aside>
          </div>
          ...
          <AnimatePresence>
            {intelligence && (
              <motion.section initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 100, damping: 15 }}
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-800 pb-5 mb-5 pl-2">
                  <div>
                    ...
                    <h3 className="text-xl font-bold text-zinc-100">"{goal}"</h3>
                  </div>
                ...
                <AnimatePresence>
                  {survivalResult && rescueResult && (
                    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="border-t border-zinc-800/60 pt-6 mt-6 pl-2 space-y-6">
