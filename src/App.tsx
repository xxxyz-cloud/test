...
const AGENTS = [
  {
    ...
    accent: "text-red-400" as const,
    icon: ShieldAlert,
    tags: ["Risk Analysis", "Gemini 2.5", "Instant"],
    tagColor: "text-red-400/90 border-red-500/25 bg-red-500/[0.07]",
    glow: "from-red-500/[0.07]",
    iconColor: "#EF4444",
    iconBg: "rgba(239,68,68,0.12)",
    hoverName: "group-hover:text-red-300",
  },
  {
    num: "02",
  ...
  },
  {
    ...
    hoverName: "group-hover:text-violet-300",
  },
];
...
// ─────────────────────────────────────────────
// AgentsShowcase — list-style layout matching phoenix_agents_showcase.html
// ─────────────────────────────────────────────
    ...
    <section className="w-full py-16 px-8 relative">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-7">
          ...
          <span className="text-[10px] font-mono tracking-[0.22em] uppercase text-red-500/80">
            The Recovery Stack
          </span>
          <div className="flex-1 h-px bg-white/[0.08]" />
        </div>
        ...
        <h2 className="text-4xl md:text-5xl font-bold tracking-tight leading-[1.1] mb-14">
          4 AI Agents.<br />
          <span className="text-zinc-700">One coordinated rescue.</span>
        </h2>
        ...
        <div className="flex flex-col">
          {AGENTS.map((agent) => {
            ...
            return (
              <div
                key={agent.num}
                className="group relative grid grid-cols-[52px_1fr_44px] gap-x-5 py-7 border-b border-white/[0.06] first:border-t first:border-white/[0.06] overflow-hidden cursor-default"
              >
                {/* Radial hover glow */}
                <div
                  className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none bg-gradient-to-r ${agent.glow} to-transparent`}
                />

                {/* Index number */}
                <span className="relative font-mono text-[11px] tracking-[0.12em] text-white/20 group-hover:text-white/45 transition-colors duration-300 pt-1.5 select-none">
                  {agent.num}
                </span>
                ...
                <div className="relative flex flex-col">
                  <span
                    className={`text-[clamp(20px,3.2vw,34px)] font-semibold tracking-[-0.03em] leading-none text-white/45 transition-colors duration-300 group-hover:text-zinc-100 ${agent.hoverName}`}
                  >
                    {agent.name}
                  </span>
                  ...
                  <p
                    className="text-[13px] leading-relaxed text-white/38 max-w-[500px] overflow-hidden transition-all duration-[380ms] ease-[cubic-bezier(0.4,0,0.2,1)]"
                    style={{
                      maxHeight: 0,
                      opacity: 0,
                      marginTop: 0,
                    ...
                    ref={(el) => {
                      ...
                      if (!parent) return;
                      const show = () => { el.style.maxHeight = "60px"; el.style.opacity = "1"; el.style.marginTop = "8px"; };
                  ...
                  {/* Tags — slides in on hover */}
                  <div
                    className="flex flex-wrap gap-1.5 overflow-hidden transition-all duration-[380ms] ease-[cubic-bezier(0.4,0,0.2,1)] delay-[50ms]"
                  ...
                  >
                    {agent.tags.map((tag, ti) => (
                      <span
                        key={ti}
                        className={`text-[9px] font-mono tracking-[0.14em] uppercase px-2 py-0.5 rounded-full border ${agent.tagColor}`}
                      >
                        {tag}
                      </span>
                    ))}
                ...
                {/* Icon */}
                <div
                  className="relative flex items-center justify-center w-10 h-10 rounded-[10px] opacity-[0.14] group-hover:opacity-95 group-hover:scale-110 transition-all duration-300 mt-0.5 flex-shrink-0"
        ...
        <div className="mt-14 grid grid-cols-3 border border-white/[0.07] rounded-2xl overflow-hidden">
          {[
            ...
            { value: "4-tier", label: "Gemini model fallback", dot: "bg-blue-500/50" },
          ].map((s, i) => (
            <div
              key={i}
              className={`relative p-6 ${i < 2 ? "border-r border-white/[0.07]" : ""}`}
            >
              <div className={`absolute top-5 right-5 w-1.5 h-1.5 rounded-full ${s.dot}`} />
              <div className="text-[26px] font-bold tracking-[-0.04em] text-zinc-100 leading-none mb-1.5">
                {s.value}
              </div>
              <div className="text-[11px] font-mono tracking-[0.1em] uppercase text-white/30">
                {s.label}
              </div>
            </div>
          ))}
...
function MainApp({ onBack, initialHours }: { onBack: () => void; initialHours?: number }) {
  ...
  React.useEffect(() => {
    if (result || survivalResult || rescueResult || simResult) {
      saveSession({
        goal, availableHours, requiredHours, progress, featuresText,
        result, survivalResult, rescueResult, simResult,
      });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [result, survivalResult, rescueResult, simResult]);
              ...
              {(result || rescueResult) && (
                <>
                  ...
                  <button
                    ...
                    onClick={() => {
                      clearSession();
                      setResult(null); setSurvivalResult(null);
                      setRescueResult(null); setSimResult(null);
                      setGoal(""); setFeaturesText(""); setProgress(0);
                      setAvailableHours(initialHours ?? ""); setRequiredHours("");
                    }}
                    className="text-[11px] font-mono text-zinc-600 hover:text-zinc-400 transition-colors flex items-center gap-1 flex-shrink-0"
                  >
        ...
        <AnimatePresence>
          {showRestoreBanner && restoredSession && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -12 }}
              className="max-w-5xl mx-auto px-4 md:px-8 mb-2 mt-2"
            >
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <span className="text-xs font-semibold text-blue-200">
                      Session recovered · {getSessionAge(restoredSession)}
                    </span>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setGoal(s.goal);
                      setAvailableHours(s.availableHours);
                  ...
                  <button
                    type="button"
                    onClick={() => { clearSession(); setShowRestoreBanner(false); }}
                    className="text-[11px] font-mono text-blue-500/60 hover:text-blue-400 transition-colors px-2"
                  >
                    Dismiss
                  </button>
                </div>
                          ...
                          {rescueResult && (
                            <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-4 mt-6">
                              ...
                              <ExportPanel
                                ...
                                keepFeatures={survivalResult?.keep || []}
                                cutFeatures={survivalResult?.cut || []}
                                successChanceBefore={survivalResult?.success_chance_before || 0}
                                successChanceAfter={survivalResult?.success_chance_after || 0}
                              ...
                              <AIMotivator
                                ...
                                riskLevel={result?.risk_level || "High"}
                                availableHours={availableHours}
                                successChanceAfter={survivalResult?.success_chance_after || 50}
                                doneCount={0}
